	...
	$("#datastreams")
			...
			.each(
					function() {
						...
						obj["dsID"] = JSON.parse($(this).find(
								"select option:selected").attr("data-value")).frostId;
						$(this)
								.find("tbody tr")
								.each(
										function() {
						...
						table.find("tbody tr").each(
								function() {
									...
									$td.eq(0).find("input").val(
											response.dateTime[i].column);
									$td.eq(1).find("input").val(
											response.dateTime[i++].string);
								});
...
function loadStreamConfig(stream, div) {
	...
	$.ajax({
		...
		success : function(result) {
			div.find("select").val(result.name + " (" + result.frostId + ")")
					.trigger("change");
			div.find("select option:selected").attr("data-value",
...
function getThings(fnSuccess) {
	...
	$.ajax({
		...
		success : function(response) {
			...
			jsonparsed
					.forEach(function(val) {
						var option = new Option(val.name + " (" + val.frostId
		...
		dropdownAutoWidth : true
			...
			function(e) {
				...
				if (e.params.data["data-value"]) {
					...
					sel.find("option:selected").attr("data-value",
							JSON.stringify(json));
				} else {
...
function loadThing(id, streams, url) {
	$.ajax({
		...
		url : "thing/single",
		data : {
			thingId : parseInt(id, 10),
			frostUrlString : url
		},
		success : function(result) {
			$("#things").val(result.name + " (" + result.frostId + ")")
					.trigger("change");
...
function loadStream(stream, div) {
	...
	for (var i = 0; i < rows; i++) {
		$(div).find("tbody").append(
				$("<tr>").append(
						$("<td>").text(stream.units_of_measurement[i].symbol))
						.append(
								$("<td>").append(
										$("<input>").attr("type", "text").attr(
												"size", "5"))));
	}
}
